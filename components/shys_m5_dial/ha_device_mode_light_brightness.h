        ...
        class HaDeviceModeLightBrightness: public esphome::shys_m5_dial::HaDeviceMode {
            protected:
                ...
                void showBrightnessMenu(M5DialDisplay& display, uint16_t currentValue){
                    ...
                    gfx->setTextColor(BrightnessMenuTextColor ); //MAROON
                    gfx->setTextDatum(middle_center);

                    gfx->startWrite();                      // Secure SPI bus

                    gfx->fillRect(0, 0, width, this->getDisplayPositionY(currentValue), BrightnessMenuBackColorOff); //RED
                    gfx->fillRect(0, this->getDisplayPositionY(currentValue), width, height, BrightnessMenuBackColorOn); //YELLOW
